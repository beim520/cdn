var textArr = ['I love three things in this world,', '我爱这个世界上的三样东西，', 'the sun ,the moon and you.', '太阳，月亮和你，', 'The sun for the day,', '白天的太阳，', 'the moon for the night,', '夜晚的月亮，', 'and you forever!', '', '永远的你！', 'If you were a teardrop,', '如果你是一滴眼泪，', 'in my eye,', '在我眼里，', 'for fear of losing you,', '因为害怕失去你，', 'I would never cry.', '我永远不会哭。', 'And if the golden sun,', '如果金色的太阳，', 'should cease to shine its light,', '如果不再闪耀光芒，', 'just one smile from you,', '你的一个微笑，', 'would make my whole world bright.', '会让我的整个世界变得光明。'];
...
function Walker(options) {
    this.step = function() {
        this.direction = Math.sign(this.target) * this.speed this.value += this.direction this.target ? this.target -= this.direction: (this.value) ? (this.wander) ? this.target = this.newTarget() : this.target = -this.value: this.target = this.newTarget() return this.direction
    }
    ...
    this.start = 0 this.value = 0 this.radius = options.radius this.target = this.newTarget() this.direction = Math.sign(this.target) this.wander = options.wander this.speed = options.speed || 1
}